...
export function ExistingInitiativeLink({ initiative }: { initiative: InitiativeSummary }) {
  const t = useTranslations("app.insights");
  return (
    <Link href={initiative.projectId || initiative.todoId ? "/roadmap" : "/diagnostic-app#insight-history"} prefetch={true} className="text-xs font-bold text-muted-foreground hover:underline">
