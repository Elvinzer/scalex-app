...
// Generic streaming chat shell, parameterized over the endpoint and its
// request body — extracted so new AI-advice features (call analysis, ad
// copy) don't each duplicate the streaming/markdown/drawer UI that
// components/improve-chat.tsx already has working for the diagnostic chat.
export function AiChatPanel({
  ...
  endpoint: string;
  buildRequestBody: (messages: ChatMessage[]) => Record<string, unknown>;
  title: string;
  ...
  async function send(history: ChatMessage[]) {
    ...
    const result = await streamChat(endpoint, buildRequestBody(history), (token) => {
      setMessages((prev) => {
        const next = [...prev];
