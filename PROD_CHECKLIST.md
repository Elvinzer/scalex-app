...
- [ ] Vérifier que le parcours fermé est visible sur `/sign-in` : le magic link reste disponible
      pour les comptes existants et le bouton Google est masqué tant que `EARLY_ACCESS_MODE=waitlist`.
...
- [ ] `EARLY_ACCESS_MODE=waitlist` tant que la liste est ouverte. Pour le lancement, passer à
      `open` après un smoke test de `/start`, `/sign-in` et `/early-access`.
