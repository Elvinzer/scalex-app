...
import type { SetterCommissions, SetterRow, SetterSaleDetail } from "./types";
...
export async function computeSetterCommissions(userId: string, setterId: string, offers: Offer[]): Promise<SetterCommissions> {
  ...
  for (const sale of setterSales) {
    ...
    commissionPaidEur += paidTotal * commissionPct;
    commissionUpcomingEur += upcomingTotal * commissionPct;
