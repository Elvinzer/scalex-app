    ...
    <>
      <div className="hidden overflow-x-auto rounded-[var(--radius-card)] border border-border bg-card p-0 md:block">
        <table className="w-full min-w-[840px] text-sm">
          ...
          <tbody>{leads.map((lead) => <tr key={lead.id} className="border-b border-border last:border-0"><td className="px-4 py-3 font-bold">{lead.platform ? sourceLabel(lead.platform) : sourceLabel(lead.source)}</td><td className="px-4 py-3"><div className="flex min-w-0 items-center gap-1"><button type="button" onClick={() => setDrawerLead(lead)} className="inline-flex min-h-11 min-w-0 items-center rounded text-left outline-none focus-visible:ring-3 focus-visible:ring-accent/20"><span className="block truncate font-bold underline-offset-2 hover:underline">{lead.displayName}</span><span className="block truncate text-xs text-muted-foreground">{lead.normalizedHandle ? `@${lead.normalizedHandle}` : lead.canonicalProfileUrl}</span></button><CrmProfileLink href={lead.canonicalProfileUrl} label={t("leads.openProfile")} iconOnly /></div></td><td className="px-4 py-3 text-muted-foreground">{lead.responsibleSetterName ?? t("detail.unassigned")}</td><td className="px-4 py-3 font-bold">{lead.contactState === "new" ? t("detail.newLead") : lead.respondedAt ? t("detail.responded") : t(CRM_OUTCOME_LABEL_KEYS[lead.outcome])}</td><td className="max-w-56 px-4 py-3 text-muted-foreground">{lead.nextAction?.title ?? t("leads.noNextAction")}</td><td className="px-4 py-3 text-muted-foreground">{createdDateFormatter.format(new Date(lead.createdAt))}</td><td className="px-4 py-3"><Button type="button" variant="outline" size="sm" className="min-h-11" onClick={() => setDrawerLead(lead)}>{t("leads.open")}</Button></td></tr>)}</tbody>
        </table>
      </div>
