...
export function CrmDisabledState({ isOwner }: { isOwner: boolean }) {
  ...
  return <section className="sticker-card flex min-h-72 flex-col items-center justify-center gap-4 p-8 text-center"><p className="text-xs font-bold tracking-[0.08em] text-muted-foreground uppercase">{t("disabledEyebrow")}</p><div className="flex size-14 items-center justify-center rounded-[var(--radius-card)] bg-accent-soft text-accent-text" aria-hidden="true">CRM</div><h2 className="text-xl font-bold">{t("disabledTitle")}</h2><p className="max-w-md text-sm text-muted-foreground">{t("disabledDescription")}</p>{isOwner ? <Button type="button" className="min-h-11" disabled={isPending} onClick={activate}>{isPending ? t("loading") : t("activate")}</Button> : <p className="text-sm font-bold text-muted-foreground">{t("ownerOnly")}</p>}{error && <p className="text-sm font-bold text-state-critical" role="alert">{error}</p>}</section>;
}
