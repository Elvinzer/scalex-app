...
export default async function CrmPipelinePage() {
  ...
  const [leads, setters, offers, closers] = await Promise.all([
    withTimeout(getCrmLeads(access.accountId), 15_000, "crm-pipeline-leads"),
    withTimeout(getCrmSetters(access.accountId), 15_000, "crm-pipeline-setters"),
    withTimeout(getBusinessSalesOfferDetails(access.accountId), 15_000, "crm-pipeline-offers"),
    withTimeout(getActiveClosers(access.accountId), 15_000, "crm-pipeline-closers"),
  ]);
