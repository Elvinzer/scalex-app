  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <form method="get" className="sticker-card grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-4 lg:items-end">
        ...
        <label className="flex flex-col gap-1 text-sm font-bold">{t("leads.allOffers")}<select name="offer" defaultValue={offerId ?? ""} className="min-h-11 rounded border border-border bg-background px-2 font-normal outline-none focus-visible:border-accent"><option value="">{t("leads.allOffers")}</option>{businessProfile.sales.offers.map((offer) => <option key={offer.id} value={offer.id}>{offer.name}</option>)}</select></label>
      ...
      {leads.length === 0 ? <p className="sticker-card p-8 text-center text-muted-foreground">{t("leads.empty")}</p> : <CrmLeadList leads={leads} setters={setters} offers={businessProfile.sales.offers} closers={closers} canAssign={hasCrmPermission(access, "crm:assign")} canManagePipeline={hasCrmPermission(access, "crm:manage-pipeline")} />}
    </div>
  );
}
