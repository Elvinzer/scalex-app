...
export default async function CrmLeadDetailPage({ params }: { params: Promise<{ leadId: string }> }) {
  ...
  if (!lead) notFound();
  return <div className="flex flex-col gap-4"><p className="text-xs font-bold tracking-[0.08em] text-muted-foreground uppercase">{t("title")}</p><CrmLeadDetail initialLead={lead} setters={setters} offers={businessProfile.sales.offers} closers={closers} canAssign={hasCrmPermission(access, "crm:assign")} canManagePipeline={hasCrmPermission(access, "crm:manage-pipeline")} /></div>;
}
