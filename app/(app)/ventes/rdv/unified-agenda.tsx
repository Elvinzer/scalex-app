...
export function UnifiedAgenda({ appointments, filters }: { appointments: Appointment[]; filters: AgendaFilters }) {
  ...
  return (
    <section className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-2" aria-label="Filtres de l’agenda">
        <div className="flex rounded-full bg-muted p-1" role="tablist" aria-label="Vue de l’agenda">
          {Object.entries(VIEW_LABELS).map(([value, label]) => <button key={value} type="button" role="tab" aria-selected={filters.view === value} onClick={() => updateUrl({ view: value })} className={`min-h-11 rounded-full px-3 text-sm font-bold ${filters.view === value ? "bg-foreground text-background" : "text-muted-foreground hover:text-foreground"}`}>{label}</button>)}
        </div>
      ...
      {appointments.length === 0 ? <div className="sticker-card-dashed flex flex-col items-center gap-2 p-10 text-center"><p className="font-bold">Aucun rendez-vous sur cette période</p><p className="text-sm text-muted-foreground">Aucune réservation ne correspond aux filtres actifs.</p><button type="button" onClick={resetFilters} className="min-h-11 text-sm font-bold text-accent underline">Réinitialiser les filtres</button></div> : filters.view === "agenda" ? <AgendaView groups={grouped} timeZone={filters.timeZone} onOpen={setDrawerAppointment} onMove={openMove} onCancel={cancelAppointment} /> : filters.view === "week" ? <WeekView appointments={appointments} timeZone={filters.timeZone} onOpen={setDrawerAppointment} /> : <ListView appointments={appointments} timeZone={filters.timeZone} onOpen={setDrawerAppointment} />}
