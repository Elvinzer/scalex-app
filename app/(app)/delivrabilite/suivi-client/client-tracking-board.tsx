      ...
      <DrawerContent className="w-[min(520px,calc(100vw-1rem))] overflow-y-auto p-5 sm:p-6">
        ...
        {client && <div className="mt-5 flex flex-col gap-6">
          ...
          {details ? <>
            ...
            {client.columnType === "success" && (
              <section className="rounded-[var(--radius-control)] border border-accent-2-border bg-accent-2-soft/60 p-3" role="status">
                <p className="text-sm font-bold">{t("client.testimonialPromptTitle")}</p>
                <p className="mt-1 text-xs leading-5 text-muted-foreground">{t("client.testimonialPromptHelp")}</p>
                <Link href={`/delivrabilite/temoignages?journeyId=${encodeURIComponent(client.id)}&clientName=${encodeURIComponent(client.clientName)}&offerId=${encodeURIComponent(client.offerId ?? "")}`} className="mt-3 inline-flex min-h-11 items-center justify-center rounded-[var(--radius-control)] border border-border bg-card px-3 text-sm font-bold hover:border-border-hover hover:bg-muted focus-visible:outline-2 focus-visible:outline-accent">{t("client.requestTestimonial")}</Link>
              </section>
            )}
...
export function ClientTrackingBoard({ initialData, initialJourneyId = null }: { initialData: DeliveryBoardData; initialJourneyId?: string | null }) {
  const t = useTranslations("deliverability");
  const [data, setData] = useState(initialData);
  const [manageColumns, setManageColumns] = useState(false);
  const [clientForm, setClientForm] = useState(false);
  const [selectedClient, setSelectedClient] = useState<DeliveryClientCard | null>(null);
  const [newSalesOpen, setNewSalesOpen] = useState(true);
  const [isPending, startTransition] = useTransition();
  ...
  useEffect(() => {
    ...
    const client = initialData.columns.flatMap((column) => column.clients).find((item) => item.id === initialJourneyId);
    if (client) setSelectedClient(client);
  }, [initialData, initialJourneyId]);
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {data.untrackedSales.length > 0 && <section className="sticker-card border-accent/40 bg-accent/5 p-4 sm:p-5"><button type="button" aria-expanded={newSalesOpen} onClick={() => setNewSalesOpen((current) => !current)} className="flex w-full items-start justify-between gap-3 text-left focus-visible:outline-2 focus-visible:outline-accent"><span className="flex flex-col gap-1"><span className="text-sm font-bold">{t("tracking.newSalesTitle")}</span><span className="text-sm text-muted-foreground">{t("tracking.newSalesSubtitle")}</span></span><ChevronDown className={`mt-0.5 size-4 shrink-0 transition-transform ${newSalesOpen ? "rotate-180" : ""}`} aria-hidden="true" /></button>{newSalesOpen && <div className="mt-4 flex flex-col gap-2">{data.untrackedSales.map((sale) => <div key={sale.id} className="flex flex-col gap-3 rounded-[var(--radius-control)] border border-border bg-card p-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-bold">{sale.clientName}</p><p className="mt-1 text-xs text-muted-foreground">{sale.offerName ?? t("client.noSale")} · {sale.totalPrice} € · {sale.saleDate}</p></div><Button type="button" variant="outline" disabled={isPending} onClick={() => startTransition(async () => { const result = await createJourneyFromSale(sale.id); if (!result.error) refresh(); })}>{t("tracking.addSale")}</Button></div>)}</div>}</section>}
