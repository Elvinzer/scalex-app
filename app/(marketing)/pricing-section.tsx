      ...
      <div className="grid items-stretch gap-5 sm:grid-cols-3">
        {PRICING_TIERS.map((tier) => (
          ...
          >
            ...
            <p className="mb-1 font-display text-lg font-bold">{t("pricing." + tier.key + ".name")}</p>
            ...
            <p className="mb-6 font-display text-[2rem] font-bold">
              {tier.price} €<span className={cn("text-[14px] font-bold", tier.highlight ? "text-white/55" : "text-muted-foreground")}>{t("pricing.perMonth")}</span>
            </p>
            <div className="mb-8 flex flex-col gap-2.5">
              {[1, 2, 3, 4].map((featureNumber) => (
                <div key={featureNumber} className="flex items-center gap-2.5 text-[13.5px]">
            ...
            >
              <a href="/onboarding">{t("pricing.choose")} {t("pricing." + tier.key + ".name")}</a>
            </Button>
          </div>
        ))}
      </div>
