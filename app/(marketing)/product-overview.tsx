  ...
  return (
    <div className="relative">
      {highlighted && (
        ...
        >
          <p className="font-semibold text-muted-foreground">{t(highlighted.labelKey)} 2026</p>
          <p className="font-display text-[15px] font-bold text-accent">
            {currencyFormatter.format(highlighted.value)}
          </p>
          <p className="text-muted-foreground">{t("product.chartTooltip")}</p>
        </div>
      )}
      ...
      <div className="flex justify-between text-[11px] text-muted-foreground">
        {points.map((p) => (
          <span key={p.labelKey}>{t(p.labelKey)}</span>
        ))}
      </div>
    </div>
  );
    ...
    <section id="produit" className="scroll-mt-24 px-6 py-16 sm:px-10 sm:py-24">
      <div className="mx-auto grid max-w-[1360px] items-center gap-14 lg:grid-cols-[0.85fr_1.15fr] lg:gap-16">
        <div>
          ...
          <p className="mb-6 max-w-xl text-[16px] leading-relaxed text-muted-foreground">
        ...
        <div className="grid gap-4 rounded-[22px] border border-border bg-white p-6 shadow-[var(--shadow-md)] sm:grid-cols-[1.3fr_1fr] sm:p-7">
          <div className="rounded-[14px] border border-border p-4">
            <div className="mb-1 flex items-center justify-between">
              <p className="text-[12px] font-semibold text-muted-foreground">
                {t("product.chartTitle")} <span className="font-normal">· {t("product.chartSubtitle")}</span>
              </p>
            </div>
            <PerformanceChart />
          </div>
          ...
          <div className="rounded-[14px] border border-border p-4">
            <p className="mb-3 text-[12px] font-semibold text-muted-foreground">{t("product.actionsTitle")}</p>
            <div className="flex flex-col gap-4">
              {PRIORITY_ACTIONS.map((action) => (
                <div key={action.labelKey} className="flex flex-col gap-1">
                  <p className="text-[13px] font-semibold text-foreground">{t(action.labelKey)}</p>
                  <div className="flex items-center gap-2">
                    <span className="text-[12px] text-muted-foreground">
                      {t("product.impact")}: {t(action.impactKey)}
                    </span>
                    <span
                      className={
