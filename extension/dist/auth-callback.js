"use strict";
const minalyCallbackStatus = document.querySelector("[data-status]");
function show(message) {
    if (minalyCallbackStatus)
        minalyCallbackStatus.textContent = message;
}
async function complete() {
    ...
    try {
        const result = await chrome.runtime.sendMessage({ type: "minaly-auth-callback", state, token, error });
