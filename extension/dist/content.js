...
function minalyPlatformLabel(platform) {
    return platform === "linkedin" ? "LinkedIn" : "Instagram";
}
function minalySourceLabel(source) {
    return { instagram: "Instagram", linkedin: "LinkedIn", tiktok: "TikTok", youtube: "YouTube", x: "X", facebook: "Facebook", email_newsletter: "Newsletter", ads: "Publicité", bouche_a_oreille: "Bouche-à-oreille", autre: "Autre" }[source] ?? source;
}
function minalyInitials(name) {
    const initials = name.split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]).join("").toUpperCase();
    return initials || "L";
}
function minalyIcon(name) {
    const paths = { check: "M5 12.5 9.5 17 19 7", close: "M6 6 18 18M18 6 6 18", external: "M14 5h5v5M19 5 11 13M17 13v5H5V6h5", lock: "M7 10V8a5 5 0 0 1 10 0v2M6 10h12v9H6z", refresh: "M20 11a8 8 0 1 0 2 5M20 5v6h-6" };
...
function minalyButton(label, className) {
    const button = minalyElement("button", label);
    button.type = "button";
    button.className = className;
    return button;
}
function minalyLabeledField(control, labelText) {
...
function minalyDetail(labelText, valueText, noteText) {
...
function minalyStatusBlock(eyebrow, title, description, statusClass) {
...
function minalyProfileCard(displayName, secondary, profileUrl = null) {
...
function minalyProfileLink(url, label = "Ouvrir le profil") {
    if (!url)
        return null;
    try {
