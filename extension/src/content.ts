...
function minalyBuildPanel(
  ...
  if (state === "loading") { panel.append(minalyElement("p", "Recherche du profil…")); return; }
  if (state === "session") {
  ...
  if (state === "unavailable") { panel.append(minalyElement("p", "Le CRM est indisponible ou désactivé pour ce compte.")); return; }
  if (state === "error") { panel.append(minalyElement("p", message ?? "Impossible de lire ce profil.")); return; }
  if (state === "success") {
  ...
  if (resolution.kind === "unknown") {
    ...
    for (const item of qualification?.offers ?? []) { const option = minalyElement("option", item.name); option.value = item.id; offer.append(option); }
    const source = document.createElement("select");
    source.className = "minaly-field";
    for (const item of qualification?.sources ?? minalyDefaultSources) { const option = minalyElement("option", item); option.value = item; option.selected = item === profile.platform; source.append(option); }
    const stage = document.createElement("select");
    stage.className = "minaly-field";
    for (const item of qualification?.stages ?? minalyDefaultStages) { const option = minalyElement("option", minalyStageLabel(item)); option.value = item; option.selected = item === "first_message_sent"; stage.append(option); }
  ...
  if (resolution.kind === "known") {
    ...
    if (resolution.lead.nextAction) panel.append(minalyElement("p", `Prochaine action : ${resolution.lead.nextAction.title} · ${minalyFormatDate(resolution.lead.nextAction.dueAt)}`));
    ...
    const stage = document.createElement("select");
    stage.className = "minaly-field";
    for (const option of minalyDefaultStages) { const item = minalyElement("option", minalyStageLabel(option)); item.value = option; item.selected = option === resolution.lead.stage; stage.append(item); }
    panel.append(stage);
...
function minalyMount(): void {
  ...
  const resolve = async () => {
    ...
    if (result.status === 403 || result.status === 503) { state = "unavailable"; return; }
  ...
  const capture = async (selection: MinalyCaptureSelection) => {
    ...
    if (result.status === 403 || result.status === 503) { state = "unavailable"; draw(); return; }
